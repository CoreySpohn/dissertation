...
% \begin{equation}
%   t_i = \frac{\textrm{SNR}^2 \cdot C_{\textrm{b}, i}}{C_{\textrm{p}, i}^2 - \left( \textrm{SNR} \cdot C_{\textrm{sp},i} \right)^2}
...
\Cref{eq:bijan_frame_duration} comes from private communication with the Roman
...
star's V magnitude affects the location where the slope changes, which can be seen in \Cref{fig:ETC_dMag_function} where the line colors represent
the V magnitude.
