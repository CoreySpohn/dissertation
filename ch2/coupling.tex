...
then in \Cref{sec:numerically_inverting_ETC} we show a numerical method for
...
has traditionally been calculated by solving \Cref{eq:2020SNR} as
\begin{equation}
...
\Cref{eq:2020rpl} represents, then solving \Cref{eq:2020rpl_expand} for
...
\Cref{eq:bijan_frame_duration} is cut to be between 1 and 80 seconds.
\Cref{eq:bijan_frame_duration} is not in literature and was pulled from a
...
\Cref{tab:eps_eri_table}. This shows that wide field imaging mode has the highest
...
are shown in \Cref{tab:top_comp_planets}.
\begin{table}
