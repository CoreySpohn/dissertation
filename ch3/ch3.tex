...
\frac{I_{\textrm{zodi}, \lambda} (\lambda_0)}{I_{\textrm{zodi}, \lambda}(\textrm{500 nm})}
  \label{eq:local_zodi}
\end{equation}
where $I_{\textrm{zodi}, \lambda}$ is the interpolant from
...
\textbf{r}}^V(\Delta \lambda_\odot, \beta_\odot)$ is from Table 17 of the same
...
observing wavelength. The units for \Cref{eq:local_zodi} are photons/s/m$^2$/as$^2$/nm
...
It further shows how the assumption on exozodiacal light impacts the
